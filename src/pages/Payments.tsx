...
export default function Payments() {
  ...
  const handlePayment = async () => {
    ...
    const payment = {
      ...
      description: `2026 PAYE - ${selectedPlanData.label} Installment`,
    };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              {isIndividual ? 'Pay your PAYE tax' : 'Pay your Company Income Tax'}
            </p>
          </div>
          
          {annualTax > 0 && (
            <div className="text-right">
              <p className="text-sm text-muted-foreground">2026 Annual Tax</p>
              <p className="text-2xl font-bold text-primary">{formatNaira(annualTax)}</p>
            </div>
          )}
        </div>
